...
/**
 * A symbol table, which is organized as a stack of scopes, maintained by
 * {@link decaf.frontend.typecheck.Namer}.
 * <p>
 ...
 * </pre>
 * 
 * Make sure the global scope is always at the bottom, and NO class scope
 * appears in neither formal nor local scope.
 *
 * @see Scope
 */
public class ScopeStack {
     ...
     * Open a scope.
     * <p>
     * If the current scope is a class scope, then we must first push all its super
     * classes and then push this. Otherwise, only push the `scope`.
     * <p>
     * REQUIRES: you don't open multiple class scopes, and never open a class scope
     * when the current scope is a formal/local scope.
     */
     ...
     * <p>
     * If the current scope is a class scope, then we must close this class and all
     * super classes. Since the global scope is never pushed to the actual
     * {@code scopeStack}, we need to pop all scopes! Otherwise, only pop the
     * current scope.
     */
    public void close() {
    ...
    /**
     * Lookup a symbol by name. By saying "lookup", the user expects that the symbol
     * is found. In this way, we will always search in all possible scopes and
     * returns the innermost result.
     *
    ...
    public Optional<Symbol> findLocalSymbol(String key, Pos pos) {
        for(int i=scopeStack.size()-1; i>=0;i--)
        {
            ...
            var symbol = localScope.find(key);
            if (symbol.isPresent() && (pos.compareTo(symbol.get().pos) > 0))
                return symbol;
        }
    ...
    /**
     * Same with {@link #lookup} but we restrict the symbol's position to be before
     * the given {@code pos}.
     *
     * @param key symbol's name
     * @param pos position
     ...
     * First, if the current scope is local scope or formal scope, then it cannot
     * conflict with any already defined symbol till the formal scope, and it cannot
     * conflict with any names in the global scope.
     * <p>
     * Second, if the current scope is class scope or global scope, then it cannot
     * conflict with any already defined symbol.
     * <p>
